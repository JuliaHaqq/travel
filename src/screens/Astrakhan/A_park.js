...
export default class A_park extends React.Component{
  ...
  render(){
    ...
    return(
      <LinearGradient 
            ...
            >
          <ScrollView>
        {Object.keys(data.astrakhan.park).map((item, index) => {
        return(
          
          <React.Fragment key={data.astrakhan.park[item].id}>
              <Modal
                animationType="slide"
                transparent={true}
              ...
              >
                  <View style={styles.modalView}>
                    <Text style={styles.modalText}>{data.astrakhan.park[item].description}</Text>
                    <Pressable
              ...
              > 
                <View>
                  <Text style={{alignItems: "center", marginBottom: 10, fontSize: 17, marginLeft: 5, marginRight: 5}}>{data.astrakhan.park[item].name}</Text>
                </View>
                <Image  source={{ uri: data.astrakhan.park[item].img }} style={{ width: 250, height: 250,  borderRadius: 10, justifyContent: "center"}} />
              </Pressable>
