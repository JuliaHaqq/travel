...
export default class U_church extends React.Component{
  ...
  render(){
    ...
    return(
      <LinearGradient 
            ...
            >
          <ScrollView>
        {Object.keys(data.ufa.church).map((item, index) => {
        return(
          
          <React.Fragment key={data.ufa.church[item].id}>
              <Modal
                animationType="slide"
                transparent={true}
              ...
              >
                  <View style={styles.modalView}>
                    <Text style={styles.modalText}>{data.ufa.church[item].description}</Text>
                    <Pressable
              ...
              > 
                <View>
                  <Text style={{alignItems: "center", marginBottom: 10, fontSize: 17, marginLeft: 5, marginRight: 5}}>{data.ufa.church[item].name}</Text>
                </View>
                <Image  source={{ uri: data.ufa.church[item].img }} style={{ width: 250, height: 250,  borderRadius: 10, justifyContent: "center"}} />
              </Pressable>
